...
class MusicPlayer:
  ...
  def __init__(self,root):
    ...
    trackframe = LabelFrame(self.root,text="",font=("Fixedsys",15,"bold"),bg="#004385",fg="#eeb1b1",bd=5,relief=FLAT)
    trackframe.place(x=0,y=400,width=600,height=100)
    # Inserting Song Track Label
    songtrack = Label(trackframe,textvariable=self.track,width=25,font=("Fixedsys",15,"bold"),bg="#033860",fg="white").grid(row=0,column=0,padx=10,pady=10)
    # Inserting Status Label
    trackstatus = Label(trackframe,textvariable=self.status,font=("Fixedsys",15,"italic"),bg="#033860",fg="white").grid(row=0,column=1,padx=10,pady=10)

    # Creating Button Frame
    buttonframe = LabelFrame(self.root,text="",font=("times new roman",15,"bold"),bg="#004385",fg="black",bd=5,relief=FLAT)
    ...
    playbtn = Button(buttonframe,text="|◀◀",command=self.prevsong,width=3,height=1,font=("times new roman",16,"bold"),
                    fg="white",bg="#004385",relief=FLAT, activebackground="#033860").grid(row=0,column=1,padx=15,pady=15)
    playbtn = Button(buttonframe,text="▶",command=self.playsong,width=2,height=1,font=("times new roman",16,"bold"),
                    fg="white",bg="#004385",relief=FLAT, activebackground="#033860").grid(row=0,column=3,padx=10,pady=15)
    # Inserting Pause Button
    playbtn = Button(buttonframe,text="||",command=self.pausesong,width=2,height=1,font=("times new roman",16,"bold"),
                    fg="white",bg="#004385",relief=FLAT, activebackground="#033860").grid(row=0,column=5,padx=10,pady=15)
    # Inserting Unpause Button
    playbtn = Button(buttonframe,text="▶||",command=self.unpausesong,width=2,height=1,font=("times new roman",16,"bold"),
                    fg="white",bg="#004385",relief=FLAT, activebackground="#033860").grid(row=0,column=7,padx=10,pady=15)
    ...
    playbtn = Button(buttonframe,text="▶▶|",command=self.nextsong,width=3,height=1,font=("times new roman",16,"bold"),
                    fg="white",bg="#004385",relief=FLAT, activebackground="#033860").grid(row=0,column=11,padx=10,pady=15)
    # Inserting Select Button
    
    # Creating Playlist Frame
    songsframe = LabelFrame(self.root,text="Song Playlist",font=("Fixedsys",18,"bold"),bg="#087CA7",fg="black",bd=5,relief=FLAT)
    ...
    but = Button(songsframe, text='play on yt',command=playonyt,bg='#033860',fg='white',relief=FLAT)
    but.pack(side=TOP,anchor=NE)
    # Inserting scrollbar
    scrol_y = Scrollbar(songsframe,orient=VERTICAL) 
    # Inserting Playlist listbox
    self.playlist = Listbox(songsframe,yscrollcommand=scrol_y.set,selectbackground="#004385",
                            selectmode=SINGLE,font=("Fixedsys",18,"bold"),bg="#05B2DC",fg="BLACK",bd=2,relief=FLAT,height=10)
    ...
    # Create Status Bar
    self.status_bar = Label(root,font=("Fixedsys",4),text='',bd=1,relief=FLAT,anchor=E,bg="#033860",fg="white")
    ...
    selectbtn = Button(self.status_bar,text="SELECT",command=self.selectfolder,width=5,height=1,font=("Fixedsys",4,"bold"),
                    fg="white",bg="#033860",relief=FLAT, activebackground="#031A6B").grid(row=0,column=4,padx=2,pady=2)
    backbtn=Button(self.status_bar,text="BACK",command=self.back,width=5,height=1,font=("Fixedsys",4,"bold"),
                    fg="white",bg="#033860",relief=FLAT, activebackground="#031A6B").grid(row=0,column=1,padx=2,pady=2)
